...
//     await fetch('https://script.google.com/macros/s/AKfycbz-boMpBTRmgQWjlkxfYDPlSA75cbbr5rTJK7mWPSGfiiTTSWES8EfBFoDudG_9wTqB/exec', {
//       method: 'POST',
//       headers: {
...
const Contact: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Create URL-encoded form data (this works better with Google Apps Script)
    ...
    formDataForSubmit.append('subject', formData.subject || 'No Subject');
    formDataForSubmit.append('message', formData.message);

    try {
      // Replace this URL with your deployed Google Apps Script web app URL
      const scriptURL = 'https://script.google.com/macros/s/AKfycbxfn9uMH9rTqVAmmk6YwhKAFUp4Xg9MQNAGQ4yB841xGSi7QxlN1VE3Pq-duC0KvmCtfA/exec';
      
      // Using fetch with form data
      fetch(scriptURL, {
        method: 'POST',
        mode: 'no-cors',
        cache: 'no-cache',
      ...
      .then(() => {
        // With no-cors we can't read the response, but if no error is thrown we assume success
        setSubmitStatus('success');
        setFormData({ name: '', email: '', subject: '', message: '' });
      })
      .catch(error => {
